...
class LeFF(nn.Module):
    ...
    def forward(self, x, x_size):
        ...
        # flaten
        x = rearrange(x, ' b c h w -> b (h w) c', h = x_size[0], w = x_size[1])
...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=4,
                 embed_dim=24, depths=[2, 2, 2, 2, 2, 2, 2, 2, 2], num_heads=[1, 2, 4, 8, 16, 16, 8, 4, 2],
    ...
    def check_image_size(self, x):
        # divisor = max down scale * window size 
        _, _, H, W = x.shape
        divisor = 16 * self.win_size
    ...
    def forward(self, x, mask=None):
        # Input Projection
        H, W = x.shape[-2:]
        y = self.check_image_size(x)
        y_size = torch.tensor(y.shape[-2:])
        
        y = self.input_proj(y)
        y = self.pos_drop(y)
        #Encoder
        conv0 = self.encoderlayer_0(y, y_size, mask=mask)
        pool0 = self.dowsample_0(conv0, y_size)
        conv1 = self.encoderlayer_1(pool0, y_size // 2, mask=mask)
        pool1 = self.dowsample_1(conv1, y_size // 2)
        conv2 = self.encoderlayer_2(pool1, y_size // 4, mask=mask)
        pool2 = self.dowsample_2(conv2, y_size // 4)
        conv3 = self.encoderlayer_3(pool2, y_size // 8, mask=mask)
        pool3 = self.dowsample_3(conv3, y_size // 8)

        # Bottleneck
        conv4 = self.conv(pool3, y_size // 16, mask=mask)

        #Decoder
        up0 = self.upsample_0(conv4, y_size // 16)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0, y_size // 8, mask=mask)
        
        up1 = self.upsample_1(deconv0, y_size // 8)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1, y_size // 4, mask=mask)

        up2 = self.upsample_2(deconv1, y_size // 4)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2, y_size // 2, mask=mask)

        up3 = self.upsample_3(deconv2, y_size // 2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3, y_size, mask=mask)

        # Output Projection
        y = self.output_proj(deconv3, y_size)[:, :, :H, :W]
        return x + y
...
if __name__ == "__main__":
    ...
    model = Uformer(use_checkpoint=False)

    H = 128
    x = torch.rand((1, 4, 137, 511))
    y = model(x)
    print(x.shape)
    print(y.shape)
    total = sum([p.nelement() for p in model.parameters() if p.requires_grad])
    print(f'parameters: {total/1e6} M')
